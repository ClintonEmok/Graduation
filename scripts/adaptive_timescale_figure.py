#!/usr/bin/env python3
"""Render a thesis-style adaptive-timescaling conceptual diagram.

Usage:
  /Users/clintonemok/miniconda3/bin/python3 scripts/adaptive_timescale_figure.py
  /Users/clintonemok/miniconda3/bin/python3 scripts/adaptive_timescale_figure.py --output-dir scripts/output/conceptual-adaptive-bin-diagram
...
DEFAULT_OUTPUT_DIR = Path('scripts/output/conceptual-adaptive-bin-diagram')
...
def build_diagram():
    ...
    # Same event loads as the fixed-bin version.
    bin_loads = [28, 4, 22, 1, 12, 40, 5, 18]
    n_bins = len(bin_loads)

    # Dense bins get more width; sparse bins compress.
    weights = [max(load, 1) ** 0.85 for load in bin_loads]
    ...
    cursor = 0.0
    for i, (load, width) in enumerate(zip(bin_loads, widths)):
        x0 = cursor
        ax.add_patch(Rectangle((x0, 0), width, 1, facecolor=BIN_FILL, edgecolor=BIN_EDGE, linewidth=1.4))
        for x, y, color in dot_positions(load, x0, width, i):
...
def main() -> None:
    parser = argparse.ArgumentParser(description='Render a conceptual adaptive-timescaling diagram.')
    parser.add_argument(
        '--output-dir',
        type=Path,
    ...
    png_path = args.output_dir / 'conceptual-adaptive-bin-diagram.png'
    svg_path = args.output_dir / 'conceptual-adaptive-bin-diagram.svg'
