...
<task type="auto">
  ...
  <verify>
    Render DataPoints component with the shader and verify:
    1. Check `renderer.info.programs.length > 0` to confirm shader program compiled.
    2. Access WebGL context and check `gl.getShaderInfoLog` for vertex and fragment shaders - should be empty.
    3. Verify no console errors related to GLSL compilation.
  </verify>
  <done>
    Shader code includes filtering logic, compiles without errors, and is active in renderer.
  </done>
</task>
