...
must_haves:
  truths:
    - "Store holds currentTime and isPlaying state"
...
<task type="auto">
  ...
  <action>
    ...
    - State: `currentTime` (number), `isPlaying` (boolean), `timeRange` ([min, max]), `speed` (number).
    - Actions: `setTime`, `setIsPlaying`, `togglePlay`, `setRange`, `setSpeed`.
    - Use `create` from 'zustand'.
    - Initialize values from `src/lib/constants.ts`.
  </action>
  <verify>grep "useTimeStore" src/store/useTimeStore.ts</verify>
