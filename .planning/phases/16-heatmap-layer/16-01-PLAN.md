...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/components/viz/SliceManagerUI.tsx"
      provides: "Heatmap toggle in the Layers menu"
---

<objective>
Establish the state management foundation for the Heatmap Layer and add the necessary UI controls to allow users to toggle the feature.
</objective>
...
<task type="auto">
  <name>Task 2: Add Heatmap Toggle to UI</name>
  ...
  <action>
    ...
    - Add a slider for 'Heatmap Radius' (linking to the store).
    - Ensure the toggle is only visible if the 'heatmap' feature flag is enabled.
    - Import and use `useHeatmapStore` and `useFeatureFlagsStore`.
  </action>
  <verify>Heatmap toggle and both sliders (Intensity, Radius) appear in the Layers menu when feature flag is on.</verify>
  <done>User can interactively toggle heatmap state and adjust intensity/radius.</done>
</task>

</tasks>

<verification>
Check that the heatmap toggle appears in the Layers panel when the 'heatmap' flag is active in Settings. Verify that intensity changes are reflected in the store.
</verification>

<success_criteria>
1. useHeatmapStore exists and correctly manages intensity, radius, and enabled state.
2. SliceManagerUI displays heatmap controls when gated by feature flag.
</success_criteria>
