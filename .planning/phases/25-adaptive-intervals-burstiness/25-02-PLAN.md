...
<task type="auto">
  ...
  <action>
    ...
    2. Replace the existing "Time Scale" toggle button with a slider:
       ...
       - Range: 0 to 1.
       - Step: 0.01.
    3. Ensure the slider fits within the layout (Option A from decision).
  </action>
  <verify>
    Sliding updates store state smoothly.
  </verify>
  <done>
    User can blend between linear and adaptive time via Timeline Header.
  </done>
</task>

<task type="auto">
  <name>Task 2: Warp the Space-Time Cube</name>
  <files>src/components/viz/SpaceTimeCube.tsx, src/components/viz/Grid.tsx</files>
  <action>
