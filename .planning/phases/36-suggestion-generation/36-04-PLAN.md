...
must_haves:
  truths:
    ...
    - "Suggestions auto-refresh when filters change (debounced 300-500ms)"
    ...
    - "Interval count from UI is passed to warp profile generation"
  artifacts:
...
<task type="auto">
  ...
  <action>
        ...
        const generateSuggestions = useCallback(async (params: GenerationParams) => {
         ...
         const timeRange = { start: startDate, end: endDate };
          ...
          const profiles = generateWarpProfiles(crimes, timeRange, { 
            profileCount: 3,
            intervalCount: params.intervalCount,  // FIX: pass user-configured interval count
          });
    ...
    5. Key behaviors:
        ...
        - FIX: Automatic re-analysis when filters change using useEffect + debounce (300-500ms). Watches crimeTypes, districts, startDate, endDate. Manual Generate button still available for immediate refresh.
        - Empty data: sets store flag for UI to display

        6. Add automatic re-analysis with useEffect:
        ```typescript
        ...
        // Watch filters and auto-trigger generation
        const filters = useViewportStore(state => state.filters);
        const startDate = useViewportStore(state => state.startDate);
        const endDate = useViewportStore(state => state.endDate);
        
        const debouncedFilters = useDebounce({ crimeTypes: filters.crimeTypes, districts: filters.districts, startDate, endDate }, 400);
        ...
        useEffect(() => {
          ...
          if (hasGeneratedOnce && generationParams) {
            handleTrigger(generationParams);
          }
        }, [debouncedFilters.crimeTypes, debouncedFilters.districts, debouncedFilters.startDate, debouncedFilters.endDate, handleTrigger]);
    ...
    7. Also add to store: setEmptyState, isEmptyState to useSuggestionStore
  </action>
  <verify>
...
- Filter changes trigger automatic re-analysis (debounced 300-500ms)
...
<success_criteria>
Real algorithm-generated suggestions integrated into the UI with all user controls per CONTEXT.md. Mock generation replaced with actual crime density analysis. Multiple alternatives offered. Suggestions respond to all filters (crime types, time range, geographic) with automatic re-analysis when filters change (debounced). Interval count from UI is passed to warp profile generation. All UI controls implemented (interval count, snapping, context display, low-confidence warnings).
</success_criteria>
