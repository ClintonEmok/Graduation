...
<task type="auto">
  ...
  <action>
    ...
    2. Create `TimeSlices.tsx` (Manager):
       ...
       - Implement `onDoubleClick` handler on an invisible "Hit Box" (PlaneGeometry matching cube bounds) to call `addSlice`.
       - IMPORTANT: For the Hit Box, use `transparent={true} opacity={0}` (do NOT use `visible={false}` as it disables raycasting).
  </action>
  <verify>
...
<task type="auto">
  ...
  <action>
    ...
    Check `useFeatureFlags` store (key: 'enableTimeSlices', default: false).
    Conditionally render `<TimeSlices />` inside the `<Scene>` component, alongside `<TimePlane />` and `<DataPoints />`, only if the feature flag is enabled.
