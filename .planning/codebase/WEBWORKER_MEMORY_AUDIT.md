...
## 1. Worker Message Payload Sizes
...
## 2. Memory Held in Workers Between Messages
...
## 3. Main Thread Computation Bottlenecks
...
## 4. Transferable Object Opportunities (MISSED)
...
```typescript
// src/workers/adaptiveTime.worker.ts line 244
...
## 6. Specific File-by-File Findings
...
| Worker lifecycle | Created per-call, terminated (useDashboardStkde.ts:84,101) | OK |
...
| DBSCAN library | density-clustering package, algorithm complexity unclear | MEDIUM |
...
|-------|----------|----------|-----------------|
| Output arrays not transferred | `adaptiveTime.worker.ts:244` | **HIGH** | Use `postMessage(obj, [buffer1, buffer2, ...])` |
...
1. **Transfer worker output arrays** — `adaptiveTime.worker.ts:244` — Easy win, ~16KB per message
...
6. **Memoize slice STKDE results** — `compute.ts:356` — Repeated work

---

*Memory audit: 2026-05-08*
