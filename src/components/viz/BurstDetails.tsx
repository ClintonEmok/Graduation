...
export function BurstDetails() {
  ...
  const stats = useMemo(() => {
    ...
    const buildStats = (window: { start: number; end: number }) => {
      const startNorm = Math.min(window.start, window.end);
      const endNorm = Math.max(window.start, window.end);
      const times: number[] = [];
    ...
    <div className="p-4 border-t">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] text-muted-foreground">
          Comparing {stats.length} bursts
        </span>
      </div>

      <div className="mt-4 space-y-3">
        {stats.map((entry, index) => (
          <div key={`${entry.window.start}-${index}`} className="rounded-md border p-3">
