...
describe('/timeslicing-algos route intent', () => {
  ...
  test('wires timeline parity behavior and compute strategy selection', () => {
    ...
    expect(shellSource).toMatch(/timestamps: contextTimestamps/);
    ...
    expect(shellSource).toMatch(/commute-heavy/);
    expect(shellSource).toMatch(/late-night-heavy/);
    expect(shellSource).toMatch(/burst-pattern/);
  ...
  test('keeps comparison-first summary while rendering detail-only bin characterization', () => {
    ...
    expect(shellSource).toMatch(/timeslicing-algos-bin-characterization-filters/);
    expect(shellSource).toMatch(/All bins/);
    expect(shellSource).toMatch(/Showing \{filteredAdaptiveBinDiagnosticsRows\.length\} of \{adaptiveBinDiagnosticsRows\.length\} bins/);
    ...
    expect(shellSource).toMatch(/No bins match the active trait filters\./);
    expect(shellSource).toMatch(/colSpan=\{4\}/);
